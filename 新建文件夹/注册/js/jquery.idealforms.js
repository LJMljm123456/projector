...
var Utils = {
  ...
  getIdealType: function( $el ) {
    ...
    return (
      /(text|password|repassword|email|number|search|tel|textarea)/.test( type ) && 'text' ||
      /file/.test( type ) && 'file' ||
  ...
  makeInput: function( name, value, type, list, placeholder ) {
    ...
    // Text & file
    if ( /^(text|password|search|url|tel|hidden)$/.test(type) )
      markup = '<input '+
        'type="'+ type +'" '+
        'id="'+ name +'" '+
        'name="'+ name +'" '+
...
var _defaults = {
  ...
  customFlags: {},
  globalFlags: '',
  onSuccess: function(e) { alert('OK！') },
  onFail: function() { alert('WGYHERSGH') },
  responsiveAt: 'auto',
  disableCustom: ''
}
